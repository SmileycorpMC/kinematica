			...
			for (String oreblock:ores) {
				if (ingot.matches(oreblock)) {
					try {
						...
						KilnRecipes.instance().addRecipe(new OreIngredient("ore"+oreblock), new OreIngredient("sand"), out);
						ItemStack out2 = out.copy();
						out2.setCount(2);
						KilnRecipes.instance().addRecipe(new OreIngredient("ore"+oreblock), new OreIngredient("dustMercury"), out);
